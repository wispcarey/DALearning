...
def train_model(epoch, loader, model_list, optimizer, scheduler, args, H_info=None):
    ...
    for batch_ind, batch_v in enumerate(loader):
        ...
        for i in range(end_ind):
            ...
            nn_input = torch.cat([obs_y.squeeze(1), ens_nn_output, ens_o_nn_output], dim=-1)
...
if __name__ == "__main__":
    ...
    args.input_dim = args.obs_dim + 2 * 64 # d + 2*64

    folder_name = os.path.join("save",
                               datetime.datetime.now().strftime('%Y-%m-%d_%H-%M'))
    folder_name += f"{args.dataset}_{args.sigma_y}_{args.N}_KST"
    ...
    # model = AttentionModel(args.input_dim, 1 + args.obs_dim * args.ori_dim).to(args.device)
    model = Simple_MLP(args.input_dim, 1 + args.obs_dim * args.ori_dim).to(args.device)
    st_model1 = SetTransformer(input_dim=args.ori_dim, num_heads=8, num_inds=64, output_dim=64, hidden_dim=64).to(args.device)
    st_model2 = SetTransformer(input_dim=args.obs_dim, num_heads=8, num_inds=64, output_dim=64, hidden_dim=64).to(args.device)
