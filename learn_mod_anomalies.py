...
from networks import ComplexAttentionModel, AttentionModel, NaiveNetwork, EquivariantTransformer, Simple_MLP
...
def train_model(epoch, loader, model_list, optimizer, scheduler, args, H_info=None):
    ...
    for batch_ind, batch_v in enumerate(loader):
        ...
        for i in range(end_ind):
            ...
            A = ens_v_f - mean_ens_v_f.unsqueeze(1)
            
            # execute model
            nn_input = torch.cat([obs_y.expand(-1, N, -1), ens_v_f, hv], dim=-1)
            nn_output = model(nn_input)
            
            # infl = 1 + nn_output[:, :1].view(B, 1, 1).expand(-1, N, D)
            # infl = 1.1
            
            # Kalman Gain
            # ens_v_a = mean_ens_v_f.unsqueeze(1).expand(-1, N, -1) + nn_output * A
            ens_v_a = nn_output * A
            # ens_v_a = model(nn_input)
...
def test_model(loader, model_list, args, verbose_test=True, H_info=None):
    model= model_list[0]
    ...
    with torch.no_grad():
        for batch_v in loader:
            ...
            for i in range(len(batch_v) - 1):
                ...
                # execute model
                nn_input = torch.cat([obs_y.expand(-1, N, -1), ens_v_f, hv], dim=-1)
                nn_output = torch.sigmoid(model(nn_input))
        ...
        return losses.avg, step_time.avg, ens_tensor
...
if __name__ == "__main__":
    ...
    args.input_dim = args.ori_dim + 2 * args.obs_dim  # d + D + d

    folder_name = os.path.join("save",
                               datetime.datetime.now().strftime('%Y-%m-%d_%H-%M'))
    folder_name += f"{args.dataset}_{args.sigma_y}_{args.N}_MA"
    ...
    # train model
    model = EquivariantTransformer(input_dim=args.input_dim, output_dim=args.ori_dim, num_heads=8, num_layers=2, dim_feedforward=64, hidden_dim=64).to(args.device)
    # model = EquivariantTransformer(input_dim=args.input_dim, output_dim=args.N, num_heads=8, num_layers=6, dim_feedforward=64, hidden_dim=64).to(args.device)
    model_list = [model]
    ...
    if args.test_only:
        ...
        test_model(test_loader, model_list, args, verbose_test=True, H_info=H_info)
